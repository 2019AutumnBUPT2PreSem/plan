...
void ChargeFun(tbl* billInfo,int IDu)
{
	...
	if(charge_cnt==1)
	{
		...
		if(charge_num==1)
		{
			...
			printf("Your account now has: %d yuan",billInfo->clm.phint[IDu][9]/10.0);
		}
...
void dial_user(tbl* teleRecord, tbl* billInfo,  int IDu)
{
	...
	if(billInfo->clm.phint[IDu][9] > 0)
	{
		...
		else
		{
			...
			else(startTime.sec>=endTime.sec)
			{
				over=0;
			}
			int totalTime = (endTime.day-startTime.day)*24*60+(endTime.hou-startTime.hou)*60+(endTime.min-startTime.min)+over;
...
void net_user(tbl* netRecord, tbl* billinfo, int IDu)
{
	...
	if(billInfo->clm.phint[IDu][9] > 0)
	{
		...
		gets();
			...
			if(billInfo->clm.phint[IDu][8] >= Time)
				{
					billInfo->clm.phint[IDu][8] = billInfo->clm.phint[IDu][8] - Time;
				}
				else
				{
					netfee = (Time - billInfo->clm.phint[IDu][8])*1;
